...
var pydict = pyData.data;

function topy(str){
	for(var i=0;i<pydict.length;i++){
		for(var n=0;n<pydict[i][1].length;n++){
			if(str == pydict[i][1].substr(n,1))
				return pydict[i][0];
		}
	}
	return str;
...
function pinyin(str, sp){
	...
	if(py.substr(py.length - 1, 1) == sp){
		return py.substr(0,py.length-1).toLowerCase()
	}
