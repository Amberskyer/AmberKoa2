...
const PinYin = Mysql.PinYin;// 将Sequelize与表结构对应
...
function initPyData() {
    pyData.forEach(async function (item) {
        let itemList = []
        item[1].split("").forEach(item2 => {
            let pyItem = {
                pinyin: item[0],
                hanzi: item2,
            }
            itemList.push(pyItem)
        })

        PinYin.bulkCreate(itemList).catch(function (err) {
            console.log("发生错误：" + err);
        });

        // let result = await Mysql.koa2.transaction(function (t) {
        //     console.log("+++++++++++++++++++");
        //     return PinYin.create(itemList, {
        //         transaction: t
        //     }).catch(function (err) {
        //         console.log("发生错误：" + err);
        //     });
